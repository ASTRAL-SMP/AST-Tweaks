...
/**
 * Screen for configuring Auto Restock protected slots.
 * Protected player-inventory slots are never used as a restock source.
 * Layout mirrors {@code AutoDropProtectedSlotScreen}: armor slots are always
 * protected (non-clickable); all other slots toggle.
 */
public class AutoRestockProtectedSlotScreen extends Screen {
    ...
    private static final int OFFHAND_SLOT = AutoRestockConfig.OFFHAND_SLOT;
    ...
    public AutoRestockProtectedSlotScreen(Screen parent) {
        super(Text.translatable("config." + ASTTweaks.MOD_ID + ".autorestock.protectedslots.title"));
        this.parent = parent;
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        Text help = Text.translatable("config." + ASTTweaks.MOD_ID + ".autorestock.protectedslots.help");
        ...
        for (int i = 0; i < ARMOR_SLOTS.length; i++) {
            ...
            drawArmorSlot(matrices, x, armorY);
        }
    ...
    private void drawLabel(MatrixStack matrices, Text text, int y) {
        this.textRenderer.drawWithShadow(matrices, text,
                gridX - 5 - this.textRenderer.getWidth(text),
                y + SLOT_SIZE / 2 - 4, 0x888888);
    }

    private void drawArmorSlot(MatrixStack matrices, int x, int y) {
